...
import { connect, Connection, makeServer } from "./index.js"


describe('websocket', ({ beforeEach, afterEach, test }) => {
  let e1: Connection, e2: Connection

  beforeEach(async () => {
    [e1, e2] = await rxjs.firstValueFrom(
      rxjs.forkJoin([
        makeServer({ port: 2033 }).pipe(
  ...
  test("keep-alive-timeout", async () => {
    expect(
      await rxjs.firstValueFrom(
        e1.keepAlive(250, 50).pipe(
          rxjs.take(3),
          rxjs.buffer(rxjs.NEVER),
          rxjs.catchError(err => rxjs.of(err))
        )
      ),
      new Expectation('instanceOf', 'TimeoutError', actual => {
